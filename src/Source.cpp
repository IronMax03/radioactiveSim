...
Source::Source(double x_, double y_, double z_,
               int rate_, double half_life_)
    : x(x_), y(y_), z(z_),
      rate(rate_), half_life(half_life_)
{}
...
std::vector<Particle> Source::emit() const {
    ...
    // Apply exponential decay: effective_rate = rate · 2^(-t/half_life)
    int effective_rate = static_cast<int>(rate * std::exp(-0.693147 * steps_elapsed / half_life));
    ++steps_elapsed;

    // rate is basically the number of particles to emit at this timestep
    for (int i = 0; i < effective_rate; ++i) {
